...
function qmckl_compute_mo_basis_mo_value_doc(context, &
...
*** Compute
   ...
   :END:
    ...
    | ~mo_num~            | ~int64_t~                      | in     | Number of MOs                                   |
    | ~point_num~         | ~int64_t~                      | in     | Number of points                                |
    | ~coefficient_t~     | ~double[ao_num][mo_num]~       | in     | Transpose of the AO to MO transformation matrix |
    ...
    #+begin_src f90 :comments org :tangle (eval f) :noweb yes
function qmckl_compute_mo_basis_mo_vgl_doc(context, &
     ao_num, mo_num, point_num, &
                          ...
                          const int64_t size_max)
{
  ...
  return QMCKL_SUCCESS;
    ...
    #+begin_src f90 :tangle (eval fh_func) :comments org :exports none
  interface
     integer(qmckl_exit_code) function qmckl_get_mo_basis_mo_hessian (context, &
          mo_hessian, size_max) bind(C)
       use, intrinsic :: iso_c_binding
       import
       implicit none

       integer (c_int64_t) , intent(in)  , value :: context
       real(c_double),       intent(out)         :: mo_hessian(*)
       integer (c_int64_t) , intent(in)  , value :: size_max
     end function qmckl_get_mo_basis_mo_hessian
  end interface
    #+end_src
...
*** Compute
   :PROPERTIES:
   :Name:     qmckl_compute_mo_basis_mo_hessian
   :CRetType: qmckl_exit_code
   :FRetType: qmckl_exit_code
   :END:

    #+NAME: qmckl_mo_basis_mo_hessian_args
    | Variable            | Type                              | In/Out | Description                                     |
    |---------------------+-----------------------------------+--------+-------------------------------------------------|
    | ~context~           | ~qmckl_context~                   | in     | Global state                                    |
    | ~ao_num~            | ~int64_t~                         | in     | Number of AOs                                   |
    ...
    | ~point_num~         | ~int64_t~                         | in     | Number of points                                |
    | ~coefficient_t~     | ~double[mo_num][ao_num]~          | in     | Transpose of the AO to MO transformation matrix |
    | ~ao_hessian~        | ~double[3][point_num][4][ao_num]~ | in     | Hessian of the AOs                              |
    | ~mo_hessian~        | ~double[3][point_num][3][mo_num]~ | out    | Hessian of the MOs                              |


    #+begin_src f90 :comments org :tangle (eval f) :noweb yes
integer function qmckl_compute_mo_basis_mo_hessian_doc_f(context, &
     ao_num, mo_num, point_num, &
     coefficient_t, ao_hessian, mo_hessian) &
     bind(C) result(info)
  use, intrinsic :: iso_c_binding
  use qmckl
  implicit none
  integer (qmckl_context), intent(in), value  :: context
  integer (c_int64_t)    , intent(in), value  :: ao_num, mo_num
  integer (c_int64_t)    , intent(in), value  :: point_num
  real    (c_double )    , intent(in)         :: ao_hessian(ao_num,4,point_num,3)
  real    (c_double )    , intent(in)         :: coefficient_t(mo_num,ao_num)
  real    (c_double )    , intent(out)        :: mo_hessian(mo_num,3,point_num,3)
  integer*8 :: i,j,k, kk, jj

  info = QMCKL_SUCCESS

  do jj=1,3
    do j=1,point_num
      mo_hessian(:,:,j,jj) = 0.d0
      do k=1,ao_num
          do kk=1,3
            do i=1,mo_num
              mo_hessian(i,kk,j,jj) = mo_hessian(i,kk,j,jj) + coefficient_t(i,k) * ao_hessian(k,kk,j,jj)
            end do
          end do
      end do
    end do
...
end function qmckl_compute_mo_basis_mo_hessian_doc_f
    #+end_src

   #+begin_src c :tangle (eval h_private_func) :comments org
   qmckl_exit_code qmckl_compute_mo_basis_mo_hessian_doc_f (
         const qmckl_context context,
         const int64_t ao_num,
         const int64_t mo_num,
         const int64_t point_num,
   ...
   :PROPERTIES:
   :Name:     qmckl_compute_mo_basis_mo_hessian_cusp
   :CRetType: qmckl_exit_code
   :FRetType: qmckl_exit_code
   :END:

    #+NAME: qmckl_mo_basis_mo_hessian_cusp_args
    | Variable        | Type                              | In/Out | Description                                     |
    |-----------------+-----------------------------------+--------+-------------------------------------------------|
    | ~context~       | ~qmckl_context~                   | in     | Global state                                    |
    ...
    #+begin_src f90 :comments org :tangle (eval f) :noweb yes
integer function qmckl_compute_mo_basis_mo_hessian_cusp_doc_f(context, &
     nucl_num, ao_num, mo_num, point_num, ao_nucl, ao_ang_mom, en_distance, &
     nucl_coord, point_coord, r_cusp, cusp_param, coefficient_t, ao_hessian, mo_hessian) &
     bind(C) result(info)
  use, intrinsic :: iso_c_binding
  use qmckl
  implicit none
  ...
  do jj=1,3
    do j=1,point_num
     ...
     mo_hessian(:,:,j,jj) = 0.d0
     do k=1,ao_num
        if (ao_hessian(k,1,j,jj) /= 0.d0) then
          inucl = ao_nucl(k)+1
        ...
        if (r < r_cusp(inucl)) then
            ...
            do kk=1,3
              do i=1,mo_num
                ...
                mo_hessian(i,kk,j,jj) = mo_hessian(i,kk,j,jj) - &
                    cusp_param(i,2,inucl) * r_vec(kk) * r_vec(jj) * r_inv**3 + &
                    cusp_param(i,4,inucl) * 3 * r_vec(kk) * r_vec(jj) * r_inv

                if (kk == jj) then
                  mo_hessian(i,kk,j,jj) = mo_hessian(i,kk,j,jj) + &
                      cusp_param(i,2,inucl) * r_inv + &
                      cusp_param(i,3,inucl) * 2 + &
                      cusp_param(i,4,inucl) * 3 * r
                end if
              end do
            end do
        end if
...
end function qmckl_compute_mo_basis_mo_hessian_cusp_doc_f
    #+end_src

   #+begin_src c :tangle (eval h_func) :comments org
   qmckl_exit_code qmckl_compute_mo_basis_mo_hessian_cusp_doc_f (
         const qmckl_context context,
...
*** Compute
   ...
   :END:
    ...
    #+begin_src f90 :comments org :tangle (eval f) :noweb yes
function qmckl_compute_mo_basis_mo_value_cusp_doc(context, &
     nucl_num, ao_num, mo_num, point_num, ao_nucl, ao_ang_mom, en_distance, &
...
*** Compute
   ...
   :END:
    ...
    #+begin_src f90 :comments org :tangle (eval f) :noweb yes
function qmckl_compute_mo_basis_mo_vgl_cusp_doc(context, &
     nucl_num, ao_num, mo_num, point_num, ao_nucl, ao_ang_mom, en_distance, &
...
//      printf("%25.15e %25.15e %d\n", mo_vgl_sp[i][4][k], mo_vgl[i][4][k], nbits);
  ...
  double * mo_hessian = (double*) malloc(mo_num*3*point_num*3 *sizeof(double));
  rc = qmckl_get_mo_basis_mo_hessian(context, &mo_hessian[0], 3*3*mo_num*point_num);
  assert(rc == QMCKL_SUCCESS);

  double * finite_difference_mo_hessian= (double*) malloc(3*3 * point_num * mo_num * sizeof(double));
  ...
  double* mo_vgls= (double*) malloc(5 * point_num * mo_num * sizeof(double));
  if (mo_vgls == NULL) {
    free(temp_coord);
    free(nucleus_coord);
    return QMCKL_ALLOCATION_FAILED;
  ...
  for (int i = 0; i < 3*3*mo_num*point_num; i++) {
    finite_difference_mo_hessian[i] = 0.0;
  }
  for (int64_t a = 0; a < nucl_num; a++) {
    for (int64_t k = 0; k < 3; k++) {
      for (int64_t m = -4; m <= 4; m++) {
        ...
        rc = qmckl_get_mo_basis_mo_vgl(context,&mo_vgls[0], 5*point_num*mo_num);
        assert(rc == QMCKL_SUCCESS);
        ...
        for (int i = 0; i < point_num; i++) {
          for (int n = 0; n < 3; n++){
            for (int j = 0; j < mo_num; j++) {
              finite_difference_mo_hessian[k*3*mo_num*point_num  + i*3*mo_num + n*mo_num + j] -=
                coef[m + 4] * mo_vgls[i*mo_num*5 + (n+1)*mo_num + j]/delta_x;
            }
          }
        }
      }
      ...
      for (int j = 0; j < point_num; j++){
        for (int m = 0; m < 3; m++){
          for (int i = 0; i < mo_num; i++){
            ...
            //printf("%.10f\n", mo_hessian[n*3*mo_num*point_num + j*3*mo_num + m*mo_num + i]);
            //printf("%.10f\n", finite_difference_mo_hessian[n*3*mo_num*point_num + j*3*mo_num + m*mo_num + i]);
            assert(fabs(finite_difference_mo_hessian[n*3*mo_num*point_num + j*3*mo_num + m*mo_num + i] - 
                        mo_hessian[n*3*mo_num*point_num + j*3*mo_num + m*mo_num + i]) < 1.e-8);
          }
        }
      }
    }
  ...
  free(mo_hessian);
  free(finite_difference_mo_hessian);
