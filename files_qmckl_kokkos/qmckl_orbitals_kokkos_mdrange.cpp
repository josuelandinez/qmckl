...
struct KokkosDeviceState {
    ...
    // Dynamic Geometry (Updates Every Call)
    ...
    // Static MO Coefficients + Host Mirror for Validation
    ...
    const KokkosDeviceState& state)
{
    ...
    Kokkos::parallel_for("AO_Gaussian_Templated", point_num, KOKKOS_LAMBDA(const int ipoint) {
        ...
        // Match qmckl_ao.c exactly: log(2^52)
            ...
            if constexpr (LMAX >= 1) {
                ...
                if constexpr (LMAX >= 3) {
                    ...
                    for (int i = 3; i <= LMAX + 2; ++i) {
                        pows[0][i] = pows[0][i - 1] * x;
                        pows[1][i] = pows[1][i - 1] * y;
                        pows[2][i] = pows[2][i - 1] * z;
                    }
    ...
    double* const ao_value) 
{
    ...
    } else if (state.lmax_global == 2) {
        kokkos_ao_gaussian_kernel<2>(point_num, ao_num, shell_num, nucl_num, state);
    } else if (state.lmax_global == 3) {
        kokkos_ao_gaussian_kernel<3>(point_num, ao_num, shell_num, nucl_num, state);
    } else if (state.lmax_global == 4) {
        kokkos_ao_gaussian_kernel<4>(point_num, ao_num, shell_num, nucl_num, state);
    } else if (state.lmax_global == 5) {
        kokkos_ao_gaussian_kernel<5>(point_num, ao_num, shell_num, nucl_num, state);
    } else {
    ...
    const double* ao_value_host, double* const mo_value) 
{
    ...
    // Universal MDRange Loop to guarantee strict IEEE-754 summation sequence 
    // to identical match the native CPU loop, resolving precision divergence
