...
struct KokkosDeviceState {
    ...
    // Static MO Coefficients (Strict LayoutRight to prevent transpose penalties)
    Kokkos::View<double**, Kokkos::LayoutRight, DeviceSpace> d_mo_coef;
    
    // Output Buffers (Strict LayoutRight to match QMCkl C API)
    ...
    Kokkos::parallel_for("AO_Gaussian_Templated", point_num, KOKKOS_LAMBDA(const int ipoint) {
        ...
        for (int inucl = 0; inucl < nucl_num; ++inucl) {
            ...
            if constexpr (LMAX >= 1) {
                ...
                // Generalized Polynomial Generator for F, G Orbitals (and beyond)
                if constexpr (LMAX >= 3) {
