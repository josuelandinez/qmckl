    ...
    Kokkos::View<double**, Kokkos::LayoutRight, DeviceSpace> d_ao_value)
{
    ...
    Kokkos::parallel_for("AO_Gaussian_Templated", point_num, KOKKOS_LAMBDA(const int ipoint) {
        ...
        // Correctly strided Structure of Arrays (SOA) memory access
        ...
        for (int inucl = 0; inucl < nucl_num; ++inucl) {
            ...
            if constexpr (LMAX >= 1) {
                ...
                // Shift applied exactly as in the native C implementation
                if constexpr (LMAX >= 2) {
                    double shift = 1.e-20;
    ...
    double* const ao_value) 
{
    ...
    if (lmax_global <= 1) {
        kokkos_ao_gaussian_kernel<1>(point_num, ao_num, shell_num, nucl_num, state.d_coord, d_nucl_coord, d_nucl_idx, d_nucl_shell_num, d_nucl_range, d_shell_ang_mom, d_shell_prim_idx, d_shell_prim_num, d_exponent, d_coef_normalized, d_ao_factor, d_ao_index, state.d_ao_value);
    } else {
        kokkos_ao_gaussian_kernel<2>(point_num, ao_num, shell_num, nucl_num, state.d_coord, d_nucl_coord, d_nucl_idx, d_nucl_shell_num, d_nucl_range, d_shell_ang_mom, d_shell_prim_idx, d_shell_prim_num, d_exponent, d_coef_normalized, d_ao_factor, d_ao_index, state.d_ao_value);
    }
    Kokkos::fence();
    ...
    if constexpr (is_cpu_backend) {
        ...
        KOKKOS_LAMBDA(const int i_pt, const int i_mo) {
            ...
            for (int k = 0; k < ao_num; ++k) {
                double ao_val = state.d_ao_value(i_pt, k);
        ...
        KokkosBlas::gemm("N", "N", 1.0, state.d_ao_value, d_mo_coef, 0.0, state.d_mo_value);
        Kokkos::fence();
    }
