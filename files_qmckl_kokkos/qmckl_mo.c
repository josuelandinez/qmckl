...
qmckl_exit_code
				     ...
				     double* const mo_value )
    {
      ...
      extern qmckl_exit_code qmckl_compute_mo_basis_mo_value_kokkos(const qmckl_context, const int64_t, const int64_t, const int64_t, const double*, const double*, double* const);
      return qmckl_compute_mo_basis_mo_value_kokkos(context, ao_num, mo_num, point_num, coefficient_t, ao_value, mo_value);
#else
...
qmckl_exit_code
qmckl_get_mo_basis_mo_hessian(qmckl_context context,
                          double* const mo_hessian,
                          const int64_t size_max)
{
  ...
  rc = qmckl_provide_mo_basis_mo_hessian(context);
  if (rc != QMCKL_SUCCESS) return rc;

  qmckl_context_struct* const ctx = (qmckl_context_struct*) context;
  assert (ctx != NULL);

  const int64_t sze = ctx->point.num * 3 * 3 * ctx->mo_basis.mo_num;
  if (size_max < sze) {
    return qmckl_failwith( context,
                           QMCKL_INVALID_ARG_3,
                           "qmckl_get_mo_basis_mo_hessian",
                           "input array too small");
  }
  memcpy(mo_hessian, ctx->mo_basis.mo_hessian, sze * sizeof(double));
...
qmckl_exit_code qmckl_provide_mo_basis_mo_hessian(qmckl_context context)
{
  ...
  if (qmckl_context_check(context) == QMCKL_NULL_CONTEXT) {
    return qmckl_failwith( context,
                           QMCKL_INVALID_CONTEXT,
                           "qmckl_provide_mo_basis_mo_hessian",
                           NULL);
  }
  ...
  /* Compute if necessary */
  if (ctx->point.date > ctx->mo_basis.mo_hessian_date) {

    qmckl_memory_info_struct mem_info = qmckl_memory_info_struct_zero;
    mem_info.size = 3 * 3 * ctx->mo_basis.mo_num * ctx->point.num * sizeof(double);

    if (ctx->mo_basis.mo_hessian != NULL) {
      qmckl_memory_info_struct mem_info_test = qmckl_memory_info_struct_zero;
      rc = qmckl_get_malloc_info(context, ctx->mo_basis.mo_hessian, &mem_info_test);

      /* if rc != QMCKL_SUCCESS, we are maybe in an _inplace function because the
         memory was not allocated with qmckl_malloc */

      if ((rc == QMCKL_SUCCESS) && (mem_info_test.size != mem_info.size)) {
        rc = qmckl_free(context, ctx->mo_basis.mo_hessian);
        assert (rc == QMCKL_SUCCESS);
        ctx->mo_basis.mo_hessian = NULL;
      }
    }
    ...
    if (ctx->mo_basis.mo_hessian == NULL) {
      ...
      double* mo_hessian = (double*) qmckl_malloc(context, mem_info);

      if (mo_hessian == NULL) {
        return qmckl_failwith( context,
                               QMCKL_ALLOCATION_FAILED,
                               "qmckl_mo_basis_mo_hessian",
                               NULL);
      }
      ctx->mo_basis.mo_hessian = mo_hessian;
    }
    ...
    if (ctx->mo_basis.r_cusp == NULL) {
      /* No cusp correction */
      rc = qmckl_compute_mo_basis_mo_hessian_doc_f(context,
                                         ctx->ao_basis.ao_num,
                                         ctx->mo_basis.mo_num,
                                         ctx->point.num,
                                         ctx->mo_basis.coefficient_t,
                                         ctx->ao_basis.ao_hessian,
                                         ctx->mo_basis.mo_hessian);
    } else {
      rc = qmckl_provide_en_distance(context);
      if (rc != QMCKL_SUCCESS) {
        return qmckl_failwith( context,
      ...
      rc = qmckl_compute_mo_basis_mo_hessian_cusp(context,
                                              ctx->nucleus.num,
                                              ctx->ao_basis.ao_num,
                                              ctx->mo_basis.mo_num,
                                              ctx->point.num,
    ...
    ctx->mo_basis.mo_hessian_date = ctx->date;
  }
...
qmckl_exit_code
qmckl_compute_mo_basis_mo_hessian_cusp (const qmckl_context context,
                                    ...
                                    double* const mo_hessian )
{
  ...
  rc = qmckl_compute_mo_basis_mo_hessian_cusp_doc_f (context, nucl_num, ao_num, mo_num, point_num,
                                               ao_nucl, ao_ang_mom, en_distance, nucl_coord,
                                               point_coord, r_cusp, cusp_param, coefficient_t,
                                               ao_hessian, mo_hessian );
